...
########################################################################
#
# If a .help file exists fro this Python script, read it in so it can be
...
def generate_stack_trace(skip_depth:int=1) -> str:
    """
    Generate a stack trace, skip displaying stack entries for this and
    the calling function as a defult.

    Arguments:
        skip_depth:
            Number of deepest functions to skip. Typically 1 to avoid
            this function from showing up in the stack dump.
...
def validate_arg_type(args_to_check) -> None:
    ...
    Arguments:
        args_to_check:  A tuple (preferred) or list, containing tuples
            (preferred) or a list that contains the argument and the
            type(s) that are valid for the argument. Ideally, these
            valid-argument-type tuples have the same order as the
            arguments in the function definition for readibility,
            consisting of:
    ...
    Returns:
        ...
        FAILURE: Prints the error(s), the stack trace and exits.
    """
    ...
    def type_xlate(xlate) -> str:  
        ...
        Returns:
            SUCCESS: A text string of the type.
            FAILURE: Print the error, the stack and exit.
        """ 
...
ArgError: Unable to translate type "{}" to string.""".format(xlate)
    ...
    def process_type(arg_off:int,
                     ...
                     one_type) -> type:
        """
        Validate type entry. Note all entries and options are validate
        to assure everything's valid in the type list.
        ...
        Returns:
            SUCCESS: The verified type
            FAILURE: Print the error that the type is NOT a valid type,
...
""".format(arg_off, type_off)
    ...
    def process_var_types(arg_off:int, var_and_valid_types) -> None:
        """
        Validate argument type table for this argument and then validate
        the argument itself against the valid types
        ...
        Returns:
            SUCCESS: Null string ("")
            FAILURE: In the case of an argument being the wrong type,
                     the error will be formatted for reporting and
                     the error string will be returned.

                     In the case of detecting an error in the validation
                     table itself, an error will be printed and the
...
def get_input(help, prompt, default=None, can_exit=True):
    """
...
def get_yes_no(help, prompt, default="Y", can_exit=True):
    """
    Print prompt that requires a YES/NO answer.
    ...
    if default not in default_prompt:
        print("""
Severe error: Default supplied for get_yes_no was "{}". It must be "Y",
              "N" or None.
""".format(default)
    ...
    #
    #Keep trying until you get a "Y", "N" "None" or an <Enter> (which
    ...
    while(True):
        ...
        if answer == "NONE":
            #
            #Answer of "None" requires we ask again, either because
            #there's no default answer, or because we can't default
            #"NONE" to "NO" (first character teh same) for safety
            #reasons.
            #
            continue

        #
        #At this point we have an input. Let's hope it's a "Y" or "N"
        #after we reduce it to the first character.
        #
...
########################################################################
#
# ADIF support variables and functions
#
########################################################################
########################################################################
...
Subnode_Enumeration = {
    ...
    "MFSK128L" : ("MFSK", None),
    "NAVTEX" : ("TOR", None),
    "OLIVIA 4/125" : ("OLIVIA", None),
    "OLIVIA 4/250" : ("OLIVIA", None),
    "OLIVIA 8/250" : ("OLIVIA", None),
    "OLIVIA 8/500" : ("OLIVIA", None),
    "OLIVIA 16/500" : ("OLIVIA", None),
    "OLIVIA 16/1000" : ("OLIVIA", None),
    "OLIVIA 32/1000" : ("OLIVIA", None),
    "OPERA-BEACON" : ("OPERA", None),
    "OPERA-QSO" : ("OPERA", None),
    "PAC2" : ("PAC", None),
    "PAC3" : ("PAC", None),
    "PAC4" : ("PAC", None),
    "PAX2" : ("PAX", None),
...
#
#Convert modes and submodes into regular expressions that can be used
#for searching.
#
re_mode_enumeration = {}
for mode in Mode_Enumeration:
    ...
    if re.search('\s', mode):
        print("""
Severe error: "{}" mode not valid. QSO modes many not have
              blanks in their names. Please contact K0RLO@nankoweap"
              about this error and send the adif.py file.
""".format(mode)) + generate_stack_trace()
        sys.exit(1)

    re_mode_enumeration[mode] = {}    
    #
    #Submodes can have blanks within them. Remove all blanks for
    #matching but keep the original format arond for readability.
    #
...
def Boolean(test):
    ...
    return("""
    ADIF boolean is not "Y" or "N" (or "y" or "n")
""")
...
def Digit(test):
    ...
    return("""
    Digit is outside the "0" to "9" (ASCII 48-57) range.
""")
...
def String(test):
    ...
    return("""
    String contains a character outside the " " to "~" (ASCII 36-126)
    range.
""")

def TBS(test):
    #
    #To be supplied checking. Placeholder, just check that it's a valid
    #string.
    #
    return(string(test))
...
def TBS_I18N(test):
    #
    #To be supplied checking. Placeholder, just return the null string
    #(no errors)
    #
    return("")
...
def SponsoredAwardList(test):
    ...
    errors = ""
    for i in test.split(","):
        if i not in Sponsored_Award_Enumeration:
            errors += """
    "{}" not a valid award.
""".format(i)
    ...
    Awards must be one of:
        "{}"
""".format(test, '"\n        "'.join(sorted(Sponsored_Award_Enumeration.keys()))))
...
def Date(test):
    ...
    if ymd:
        ...
        if month > 12:
            errors += """
    ADIF date's month is greater than 12: "{}"
""".format(month)
        ...
        if day > maxday:
            errors += """
    ADIF date's day is greater than the days in the month.
...
def Ant_Path(test):
    ...
    if test in Ant_Path_Enumeration:
        return("")
...
def Location(test):
    ...
    where X is "N","S", "E" or "W" and DDD is 0-180 with leading zeroes,
    one space, then MM.MMM is 00.000-59.999 with leading and trailing
    zeroes.
""".format(test))
...
def Arrl_Sect(test):
    ...
    if test in ARRL_Section_Enumeration:
        return("")
...
def QSO_Upload_Status(test):
    ...
    if test in QSO_Upload_Status_Enumeration:
        return("")
...
def Continent(test):
    ...
    if test in Continent_Enumeration:
        return("")
...
def Dxcc(test):
    ...
    if test in DXCC_Entity_Code_Enumeration:
        return("")
...
def QSL_Rcvd(test):
    ...
    if test in QSL_Rcvd_Enumeration:
        return("")
...
def QSL_Sent(test):
    ...
    if test in QSL_Sent_Enumeration:
        return("")
...
def QSL_Via(test):
    ...
    if test in QSL_Via_Enumeration:
        return("")
...
def WWFFRef(test):
    ...
    if re.fullmatch(r'[A-Z0-9]{1,4}FF-\d{4}', test):
        return("")
...
QSO_fields = {
    ...
    "CNTY" : (TBS),
    "COMMENT" : (String),
    "COMMENT_INTL" : (TBS_I18N),
    "CONT" : (Continent),
    "CONTACTED_OP" : (String),
    "CONTEST_ID" : (Contest),
    "COUNTRY" : (String),
    "COUNTRY_INTL" : (TBS_I18N),
    "CQZ" : (PositiveInteger),
    ...
    "MS_SHOWER" : (String),
    "MY_ANTENNA" : (String),
    "MY_ANTENNA_INTL" : (TBS_I18N),
    "MY_ARRL_SECT" : (Arrl_Sect),
    "MY_CITY" : (String),
    "MY_CITY_INTL" : (TBS_I18N),
    "MY_CNTY" : (TBS),
    "MY_COUNTRY" : (String),
    "MY_COUNTRY_INTL" : (TBS_I18N),
    ...
    "MY_LON" : (TBS),
    "MY_NAME" : (String),
    "MY_NAME_INTL" : (TBS_I18N),
    "MY_POSTAL_CODE" : (String),
    "MY_POSTAL_CODE_INTL" : (TBS_I18N),
    "MY_RIG" : (String),
    "MY_RIG_INTL" : (TBS_I18N),
    "MY_SIG" : (String),
    "MY_SIG_INFO" : (String),
    "MY_SIG_INFO_INTL" : (TBS_I18N),
    "MY_SIG_INTL" : (TBS_I18N),
    "MY_SOTA_REF" : (TBS),
    "MY_STATE" : (TBS),
    "MY_STREET" : (String),
    "MY_STREET_INTL" : (TBS_I18N),
...
def record(tags, tag_order=None):
    ...
    for tag_name in tag_order:
        if tag_name in tags:
            ...
            #
            #Remove the tag from the dictonary to see if we have any
            #leftovers when we're done, which would be a severe error.
            #
            del tags[tag_name]
    ...
    if tags:
        print("""
Severe error: The following tags were left over after creating the ADIF
              record:
              {}
...
def valid_mode(mode_submode):
    ...
    #
    #Pull off mode and re-create submode in case they're blanks in it
    #(like the OLIVIA submodes have)
    #
    ...
    #
    for (mode, submode) in [[mode_text, submode_text],
        [mode_text + submode_text, ""]]:
        if submode:
            #
            ...
            if mode in re_mode_enumeration:
                #
                #Mode looks good, see if this is a valid submode for
                #that mode
                #
            ...
            if mode in re_mode_enumeration:
                #
                #Only a valid mode is specified. Return it and no
                #submode
                #
                return((mode,))
            #
            #Did not find a mode match, do an exhaustive search through
            #all the submodes for a match
            #
            for mode_index in re_mode_enumeration.keys():
                if mode in re_mode_enumeration[mode_index]:
                    return((mode_index,
                        re_mode_enumeration[mode_index][mode]))
